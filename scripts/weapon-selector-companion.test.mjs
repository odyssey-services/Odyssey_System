...
//
// Bug: the weapon-selector companion popover showed "No weapons available" even
// though the Gun HUD displayed the active weapon. Root cause: the companion fed
// the RAW broadcast payload straight into renderWeaponSelectorPanel, which reads
// `state.snapshot.weapon.available`. The broadcast payload carries the weapon
// view model at `hudSnapshot.weapon.available` — the `snapshot` shape only
// exists AFTER buildSyntheticState (the path the Gun module already uses). So
// `state.snapshot` was undefined → empty list → false "No weapons available".
//
...
// Armory with a MELEE weapon (no caliber, no magazine) + two ranged weapons.
function mixedArmory() {
  return {
    ok: true,
    weapons: [
      {
        id: "w-katana", name: "Plasma Katana",
        model: { weapon_class_name: "Melee" }, // NOTE: no caliber → melee
        active_profile_id: "pk", active_profile: { id: "pk", name: "Edge" },
      },
      {
        id: "w-rifle", name: "Standard Rifle",
        ...
        loaded_magazine: { id: "mr", current_rounds: 30, capacity: 30, ammo_type_name: "FMJ", magazine_def: { capacity: 30, caliber: "5.56" } },
      },
      {
        id: "w-pistol", name: "Frontier Pistol",
        ...
        loaded_magazine: { id: "mp", current_rounds: 12, capacity: 12, ammo_type_name: "exp", magazine_def: { capacity: 12, caliber: ".45" } },
      },
    ],
    magazines: [
...
// A raw BC_HUD_SELECTION payload exactly as buildBroadcastPayload emits it:
// weapon data lives under `hudSnapshot`, NOT `snapshot`.
...
// ─── 1. hudSnapshot carries the real weapon list ────────────────────────────
test("broadcast payload exposes 3 weapons under hudSnapshot.weapon.available", () => {
  ...
  assert.equal(p.snapshot, undefined, "raw payload has NO `snapshot` key (only hudSnapshot)");
});

// ─── 2. Companion bridges hudSnapshot → snapshot (same count as Gun module) ──
test("companion selector state exposes the same weapon count as the Gun module", () => {
  ...
  assert.ok(selState, "companion state built (snapshot present)");
  assert.equal(
    selState.snapshot.weapon.available.length,
    p.hudSnapshot.weapon.available.length,
    "companion count === Gun-module (hudSnapshot) count",
  );
...
test("REGRESSION: raw payload (hudSnapshot, no snapshot) still yields weapons", () => {
  const p = readyPayload(mixedArmory());
  // Old broken path read p.snapshot.* (undefined) → []. Fixed path normalizes.
  const selState = buildCompanionSelectorState(p);
...
// ─── 3 & 4. Melee / no-caliber weapons are NOT filtered out ─────────────────
...
// ─── 6. Genuine empty only after a successful empty load ────────────────────
...
// ─── 7. Opening right after ready selection is not a false empty ────────────
...
// ─── 8. Selecting a weapon updates the primary view model ───────────────────
test("selectedWeaponId picks a non-default primary and marks it in the list", () => {
  const def = readyPayload(mixedArmory()); // no selection → primary is weapons[0]
  assert.equal(def.hudSnapshot.weapon.primary.id, "w-katana");

  const chosen = readyPayload(mixedArmory(), { selectedWeaponId: "w-pistol" });
  assert.equal(chosen.hudSnapshot.weapon.primary.id, "w-pistol", "Gun primary follows selection");
  const selState = buildCompanionSelectorState(chosen);
  const marked = selState.snapshot.weapon.available.find((w) => w.selected);
  assert.equal(marked?.id, "w-pistol", "selector marks the selected weapon");
  const html = renderWeaponSelectorPanel(selState);
  assert.ok(html.includes("is-selected"));
});

// ─── 9. Closing the selector does not clear the weapon list ─────────────────
...
// ─── Magazine companion shares the same normalization ───────────────────────
test("magazine companion: null state → loading, ready state → reads snapshot", () => {
  ...
  const selState = buildCompanionSelectorState(readyPayload(mixedArmory(), { selectedWeaponId: "w-rifle" }));
