...
function buildBroadcastPayload(state, ephemeral = {}) {
  ...
  return {
    ...
    ui: {
      ...
      weaponSelectorOpen: !!ephemeral.weaponSelectorOpen,
      weaponDataLoading: !!ephemeral.weaponDataLoading,
...
var WEAPON_HEAVY_CACHE_STALE_MS = 1e4;
function setupSceneSelection(hooks = {}) {
  ...
  const cleanups3 = [];
  const queuedCombatActionKeys = /* @__PURE__ */ new Set();
  const queuedCombatActionCharacters = /* @__PURE__ */ new Set();
  ...
  async function runCharacterActionQueue(characterId, work, {
    ...
    if (!normalizedCharacterId || typeof work !== "function") {
      return typeof work === "function" ? work() : void 0;
    }
    ...
    const runPromise = (previous ?? Promise.resolve()).catch(() => {
    }).then(async () => work());
  ...
  async function waitForCombatActionIdle(characterId, timeoutMs = 3e3) {
    const normalizedCharacterId = String(characterId ?? "").trim();
    if (!normalizedCharacterId) return;
  ...
  function applyHeavyCacheToLastReadyState(characterId) {
    const normalizedCharacterId = String(characterId ?? "").trim();
    if (!normalizedCharacterId || lastState?.status !== "ready" || lastState?.access?.canView !== true || String(lastState?.characterId ?? "").trim() !== normalizedCharacterId || !lastState.runtimeBundle) {
      return false;
    }
  ...
  function normalizeSelectionClassification(selection) {
    return Array.isArray(selection) ? selection.map((entry) => ({
  ...
  function sceneItemLooksLikeHudPreview(item) {
    ...
    const metadataKeys = item?.metadata && typeof item.metadata === "object" ? Object.keys(item.metadata).map((key) => String(key ?? "").trim().toLowerCase()) : [];
    return layer === "POINTER" || layer === "POPOVER" || type === "TEXT" || itemId.startsWith("odyssey-") || metadataKeys.some((key) => key.startsWith("com.odyssey"));
  }
    ...
    for (const selectionId of normalizedSelectionIds) {
      ...
      try {
        tokenLinkResult = await getSceneTokenLinks({
          ...
          campaign_id: context.campaignId || void 0,
          token_id: selectionId
        }, settings);
      } catch {
        tokenLinkResult = null;
      }
      const link = Array.isArray(tokenLinkResult?.links) ? tokenLinkResult.links.find((entry) => String(entry?.token_id ?? "").trim() === selectionId && entry?.is_active !== false) : null;
  ...
  function isHudInteractionSelectionStickyActive() {
    return lastPayload?.status === "ready" && !!lastPayload?.characterId && (ephemeral.weaponSelectorOpen || ephemeral.magazineSelectorOpen || ephemeral.fireModeSelectorOpen || ephemeral.weaponSwitchInFlight || ephemeral.reloadInFlight || ephemeral.fireModeInFlight || ephemeral.targeting?.mode === "picking" || movementPreviewActive === true);
  }
  function shouldIgnoreUnlinkedSelectionNoise(selectionIds, classification, reason = "selection-changed") {
    ...
    const onlyUnlinkedOrNonCharacter = Array.isArray(classification) && classification.length > 0 && classification.every((entry) => entry?.kind === "unlinked-token" || entry?.kind === "drawing-item" || entry?.kind === "hud-preview-item" || entry?.kind === "unknown-non-token");
  ...
  function shouldIgnoreNonCharacterSelection(classification) {
    const hasReadyCharacter = lastPayload?.status === "ready" && !!lastPayload?.characterId;
    return hasReadyCharacter && Array.isArray(classification) && classification.length > 0 && classification.every((entry) => entry?.kind === "drawing-item" || entry?.kind === "hud-preview-item" || entry?.kind === "unknown-non-token");
  }
  ...
  async function handleObservedNonEmptySelection(selectionIds, reason = "selection-changed") {
    ...
    const linkedSelectionIds = classification.filter((entry) => entry.kind === "linked-character-token").map((entry) => entry.id);
    const unlinkedSelectionIds = classification.filter((entry) => entry.kind === "unlinked-token").map((entry) => entry.id);
    ...
    const nextSelectionIds = linkedSelectionIds.length > 0 ? linkedSelectionIds : unlinkedSelectionIds.length > 0 ? unlinkedSelectionIds : [];
  ...
  async function init() {
    ...
    const sessionController = configured ? setupCombatSessionController({
      ...
      onSessionRuntime: (runtime) => {
        ...
        if (lastState) broadcastReadyStateUpdate2(["session"], "session-runtime-loaded");
    ...
    if (sessionController) {
      const unsubscribeMoveTool = await subscribeMoveToolMessages((event) => {
        if (event.type === MOVE_TOOL_EVENTS.Status) {
          movementPreviewActive = event?.payload?.active === true || event?.payload?.pending === true || !!event?.payload?.preview;
          return;
        }
    ...
    const quickbarController = configured ? setupQuickbarController({
      ...
      onRuntime: (runtime) => {
        ...
        if (lastState?.status === "ready" && lastState?.access?.canView === true) {
          broadcastReadyStateUpdate2(["skills"], "abilities-runtime-loaded");
        } else if (lastState) {
    ...
    function currentMappedSession2() {
    ...
    async function fetchLightRuntimeBundle(characterId, reason = "selection-runtime") {
      const selectedSession = characterId && characterId === ephemeral.characterId ? currentMappedSession2() : null;
    ...
    async function refreshHeavyCharacterData2(characterId, {
        ...
        transientEmptySelectionTimer = setTimeout(async () => {
          ...
          if (liveSelectionIds.length === 0 && pendingSelectionIds.length === 0) {
            if (isHudInteractionSelectionStickyActive()) {
              ...
              replayLastVisibleState2(`${reason}:hud-interaction-active`);
              return;
            }
    ...
    function isCurrentSource2(characterId, selectedItemId) {
    ...
    function replayLastVisibleState2(reason = "state-update") {
      if (lastState) {
        return publishState(lastState, reason);
      }
      if (lastPayload) {
    ...
    function broadcastReadyStateUpdate2(scopes, reason = "module-update") {
    ...
    async function refetchCurrent(reason = "generic") {
      ...
      refetchCurrentPromise = (async () => {
        const now = Date.now();
        const waitMs2 = Math.max(0, 350 - (now - lastRefetchAt));
        if (waitMs2 > 0) {
          await new Promise((resolve) => setTimeout(resolve, waitMs2));
        }
    ...
    async function refreshSelectedCharacterRuntime(reason = "generic", { refreshQuickbar = false, insideCharacterQueue = false } = {}) {
      ...
      const encounterId = currentMappedSession2()?.id ?? null;
      ...
      const performRefresh = async () => {
        ...
        try {
          ...
          if (finalPublishReason && lastState?.status === "ready" && lastState?.access?.canView === true && String(lastState?.characterId ?? "").trim() === characterId) {
            broadcastReadyStateUpdate2(scopesForRuntimeRefresh(reason, refreshQuickbar), finalPublishReason);
          }
        }
      };
    ...
    function applyTargetingPayload(payload) {
      ...
      broadcastReadyStateUpdate2(["targeting"], "targeting-updated");
    }
    ...
    async function handleCommand(command) {
      ...
      if (isCombatSyncBlockedCommand(command)) {
        ...
        broadcastReadyStateUpdate2(["session", "weapon", "skills"], "command-blocked:combat-sync");
        return;
      }
      ...
      if (command?.scope === "combat-hud" && command?.feature === "fire-mode") {
        ...
        if (fmType === "select") {
          ...
          try {
            ...
            const heavyEncounterId = currentMappedSession2()?.id ?? null;
            await refreshHeavyCharacterData2(ephemeral.characterId, {
      ...
      if (command?.scope === "combat-hud" && command?.feature === "quickbar" && command?.type === "execute-direct-ability") {
        ...
        const sessionAtRequest = currentMappedSession2();
      ...
      if (type === "select-weapon") {
        ...
        const session = currentMappedSession2();
        ...
        if (!weaponId || !characterIdAtRequest) {
          ...
          replayLastVisibleState2("weapon-switch-unavailable");
          return;
        }
        if (weaponId === activeWeaponId) {
          replayLastVisibleState2("weapon-switch-same-weapon");
          return;
        }
        if (selectedOption?.switchAllowed === false) {
          ...
          replayLastVisibleState2("weapon-switch-blocked");
          return;
        }
        ephemeral.selectedReloadMagazineId = null;
        ephemeral.reloadRpcResult = null;
        ...
        try {
          ...
          await refreshHeavyCharacterData2(characterIdAtRequest, {
          ...
          if (!isCurrentSource2(characterIdAtRequest, selectedItemIdAtRequest)) {
          ...
          if (shouldPublishAfterFinally && isLatestWeaponSwitch && isCurrentSource2(characterIdAtRequest, selectedItemIdAtRequest)) {
            ...
            if (lastState?.status !== "ready") {
              ...
              broadcastReadyStateUpdate2(scopesForBroadcastReason(finalPublishReason), finalPublishReason);
            }
          }
        }
      }
      if (type === "toggle-weapon-selector") {
        ...
        const encounterIdAtOpen = currentMappedSession2()?.id ?? null;
        broadcastReadyStateUpdate2(
          ["weapon"],
        ...
        if (ephemeral.weaponSelectorOpen && characterIdAtOpen) {
          ...
          broadcastReadyStateUpdate2(["weapon"], "weapon-selector-cache-applied");
          if (isWeaponHeavyCacheStale(characterIdAtOpen, encounterIdAtOpen)) {
            ephemeral.weaponDataLoading = true;
            broadcastReadyStateUpdate2(["weapon"], "weapon-selector-refreshing");
            void refreshHeavyCharacterData2(characterIdAtOpen, {
              reason: "weapon-selector-opened",
              encounterId: encounterIdAtOpen,
              armory: true,
              inventory: true
            }).then(() => {
              if (!isCurrentSource2(characterIdAtOpen, selectedItemIdAtOpen)) return;
              if (!applyHeavyCacheToLastReadyState(characterIdAtOpen)) return;
              ephemeral.weaponDataLoading = false;
              broadcastReadyStateUpdate2(["weapon"], "weapon-data-loaded");
            }).catch((error) => {
              ephemeral.weaponDataLoading = false;
              ...
              broadcastReadyStateUpdate2(["weapon"], "weapon-data-load-failed");
            });
          }
        }
        return;
      ...
      if (type === "close-weapon-selector") {
        ephemeral.weaponSelectorOpen = false;
        ephemeral.weaponDataLoading = false;
        broadcastReadyStateUpdate2(["weapon"], "weapon-selector-closed");
        return;
      }
      if (type === "toggle-magazine-selector") {
        ...
        broadcastReadyStateUpdate2(["weapon"], ephemeral.magazineSelectorOpen ? "magazine-selector-opened" : "magazine-selector-closed");
        if (ephemeral.magazineSelectorOpen && characterIdAtOpen) {
          applyHeavyCacheToLastReadyState(characterIdAtOpen);
          broadcastReadyStateUpdate2(["weapon"], "magazine-selector-cache-applied");
          if (isWeaponHeavyCacheStale(characterIdAtOpen, encounterIdAtOpen)) {
            ephemeral.weaponDataLoading = true;
            broadcastReadyStateUpdate2(["weapon"], "magazine-selector-refreshing");
            void refreshHeavyCharacterData2(characterIdAtOpen, {
              ...
              if (!applyHeavyCacheToLastReadyState(characterIdAtOpen)) return;
              ephemeral.weaponDataLoading = false;
              broadcastReadyStateUpdate2(["weapon"], "magazine-data-loaded");
            }).catch((error) => {
              ephemeral.weaponDataLoading = false;
              ...
              broadcastReadyStateUpdate2(["weapon"], "magazine-data-load-failed");
            });
          }
        }
        return;
      ...
      if (type === "select-reload-mag") {
        ...
        broadcastReadyStateUpdate2(["weapon"], "magazine-selected");
        return;
      }
      ...
      if (type === "reload") {
        ...
        const reloadSession = currentMappedSession2();
...
// movement/drawingObstacles.js
var DRAWING_OBSTACLE_TOLERANCE_PX = 4;
var MOVEMENT_COLLISION_EPSILON = 1e-3;
...
function getStrokeWidth(item) {
  return Math.max(
    0,
    toNumber(
      item?.style?.strokeWidth ?? item?.strokeWidth ?? item?.style?.lineWidth ?? item?.lineWidth,
      0
    )
  );
}
...
function rectToPolygon(bounds, rotationDegrees = 0) {
  if (!bounds) return [];
  const points = [
...
function ellipseToPolygon(center, radiusX, radiusY, segments = 32, rotationDegrees = 0) {
...
function rawPointsFromItem(item) {
  const arrays = [
    ...
    item?.path,
    item?.vertices,
    Array.isArray(item?.commands) ? item.commands.map((command) => normalizePoint(command)).filter(Boolean) : null
  ];
...
function maybeOffsetLocalPoints(points, item) {
  ...
  const localLike = width > 0 && height > 0 ? spanX <= width * 1.5 + MOVEMENT_COLLISION_EPSILON && spanY <= height * 1.5 + MOVEMENT_COLLISION_EPSILON : Math.max(...xs.map((value) => Math.abs(value)), 0) < 500 && Math.max(...ys.map((value) => Math.abs(value)), 0) < 500;
...
function closePolygon(points) {
  ...
  if (Math.abs(first.x - last.x) <= MOVEMENT_COLLISION_EPSILON && Math.abs(first.y - last.y) <= MOVEMENT_COLLISION_EPSILON) {
...
function collectOwlbearDrawingObstacles(sceneItems, options = {}) {
  const excludeItemIds = new Set(
    Array.isArray(options?.excludeItemIds) ? options.excludeItemIds.map((value) => String(value ?? "").trim()).filter(Boolean) : []
  );
...
function onSegment(a, b, c) {
  return b.x <= Math.max(a.x, c.x) + MOVEMENT_COLLISION_EPSILON && b.x + MOVEMENT_COLLISION_EPSILON >= Math.min(a.x, c.x) && b.y <= Math.max(a.y, c.y) + MOVEMENT_COLLISION_EPSILON && b.y + MOVEMENT_COLLISION_EPSILON >= Math.min(a.y, c.y);
}
...
function segmentNearSegment(a1, a2, b1, b2, tolerance = 0) {
  if (segmentsIntersect(a1, a2, b1, b2)) return true;
  const limit = Math.max(0, toNumber(tolerance, 0));
  if (limit <= MOVEMENT_COLLISION_EPSILON) return false;
  return distancePointToSegment(a1, b1, b2) <= limit + MOVEMENT_COLLISION_EPSILON || distancePointToSegment(a2, b1, b2) <= limit + MOVEMENT_COLLISION_EPSILON || distancePointToSegment(b1, a1, a2) <= limit + MOVEMENT_COLLISION_EPSILON || distancePointToSegment(b2, a1, a2) <= limit + MOVEMENT_COLLISION_EPSILON;
}
function pointInPolygon(point, polygon) {
  ...
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i, i += 1) {
    ...
    const intersects = pi.y > point.y !== pj.y > point.y && point.x < (pj.x - pi.x) * (point.y - pi.y) / (pj.y - pi.y || MOVEMENT_COLLISION_EPSILON) + pi.x;
    if (intersects) inside = !inside;
  }
  return inside;
}
function segmentIntersectsPolygon(segmentStart, segmentEnd, polygon, tolerance = 0) {
...
function findBlockingDrawingObstacleForPath({ path, grid, obstacles }) {
...
function setupTacticalMoveTool({ runtime }) {
  ...
  function collectDrawingObstaclesForMovement() {
    ...
    const summary = {
      ...
      kinds: [...new Set(
        obstacles.map((obstacle) => String(obstacle?.obstacleKind ?? obstacle?.kind ?? "").trim()).filter(Boolean)
      )].sort()
    };
