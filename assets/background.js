...
var WEAPON_HEAVY_CACHE_STALE_MS = 6e4;
function setupSceneSelection(hooks = {}) {
  ...
  function getDisplayedActiveWeaponId() {
    return String(
      lastPayload?.hudSnapshot?.weapon?.primary?.id ?? ephemeral.selectedWeaponId ?? ""
    ).trim() || null;
  }
  function getAbilityWeaponRequirementReason(action, selectedWeaponId = getDisplayedActiveWeaponId()) {
    const requirements = action?.requirements && typeof action.requirements === "object" ? action.requirements : {};
    const sourceWeaponId = String(
      action?.sourceCharacterWeaponId ?? requirements.weaponId ?? ""
    ).trim() || null;
    ...
    return action?.state?.disabledReason || requirements.conditionSummary || (action?.sourceLabel ? `Select ${action.sourceLabel}.` : "Select the required weapon.");
  }
  ...
  function hasRenderableWeaponSnapshot() {
    ...
    return !!weapon && (!!weapon.primary || Array.isArray(weapon.available) || Array.isArray(weapon.primary?.reserveMagazines));
  }
  ...
  async function init() {
    ...
    async function refreshHeavyCharacterData(characterId, {
      ...
      if (nextPatch.armory && Number.isFinite(previousUpdatedAt) && previousUpdatedAt > refreshStartedAt) {
    ...
    async function applyStableSelectionState(resolvedState) {
      ...
      try {
        ...
        const normalized = normalizeRpcError(error);
        const alreadyPublishedReadyForThisRequest = lastPayload?.status === SELECTION_STATUS.ready && String(lastPayload?.characterId ?? "").trim() === String(resolvedState?.characterId ?? "").trim() && String(lastPayload?.selectedItemId ?? "").trim() === String(resolvedState?.tokenId ?? "").trim();
    ...
    async function handleCommand(command) {
      ...
      if (type === "select-weapon") {
        ...
        try {
          ...
          const authoritativeArmory = result?.armory && typeof result.armory === "object" ? result.armory : null;
          const authoritativeActiveWeaponId = String(
            result?.active_weapon_id ?? authoritativeArmory?.active_weapon_id ?? weaponId
          ).trim() || null;
          ...
          if (shouldPublishAfterFinally && isLatestWeaponSwitch && isCurrentSource(characterIdAtRequest, selectedItemIdAtRequest)) {
    ...
    cleanups3.push(lib_default.broadcast.onMessage(BC_HUD_SELECTION_REQUEST, (event) => {
      ...
      if (forceReplay === true && lastPayload?.status === "ready" && lastPayload?.selectedItemId) {
        replayLastVisibleState2("companion-force-replay");
        return;
      }
...
async function replaySelectionToCompanion(moduleId, reason) {
  ...
  try {
    await lib_default.broadcast.sendMessage(BC_HUD_SELECTION, lastSelectionPayload, {
      destination: "LOCAL"
    });
