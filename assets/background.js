...
var MOVE_TOOL_ICON_URL = "https://odyssey-services.github.io/Odyssey_System/icon.svg?v=1.8.53";
...
function createInitialState() {
  return {
    ...
    gridRecoveryKey: "",
    blockedShapeCells: /* @__PURE__ */ new Map(),
...
function collectBlockedCellsFromShapes(items, grid) {
  const blocked2 = /* @__PURE__ */ new Map();
  ...
  for (const item of ensureArray3(items)) {
    ...
    const maxQ = Math.floor((Math.max(bounds.right - 1e-3, bounds.left) - anchorX) / gridDpi);
    const minR = Math.floor((bounds.top - anchorY) / gridDpi);
    const maxR = Math.floor((Math.max(bounds.bottom - 1e-3, bounds.top) - anchorY) / gridDpi);
    for (let q = minQ; q <= maxQ; q += 1) {
      for (let r = minR; r <= maxR; r += 1) {
        const cellKey = getCellKey({ q, r });
        if (!blocked2.has(cellKey)) {
          blocked2.set(cellKey, {
