...
function renderGunBlock(state) {
  ...
  const weaponList = availableWeapons.length > 1 ? `<div class="ohud-weapon-list">${availableWeapons.map(weaponOption).join("")}</div>` : "";
  const reserveList = reserve.length ? `<div class="ohud-reserve-list">${reserve.map(reserveOption).join("")}</div>` : "";
