...
drop function if exists public.perform_attack(jsonb);
drop function if exists public.odyssey_refresh_character_combat_state(uuid);
drop function if exists public.odyssey_trim_combat_log(uuid, text);

alter table public.odyssey_weapon_model_defs
  add column if not exists base_melee_damage integer not null default 0;

alter table public.odyssey_weapon_model_defs
  drop constraint if exists odyssey_weapon_model_defs_base_melee_damage_check;

alter table public.odyssey_weapon_model_defs
  add constraint odyssey_weapon_model_defs_base_melee_damage_check
  check (base_melee_damage >= 0);
...
alter table public.odyssey_character_body_parts
  drop constraint if exists odyssey_character_body_parts_armor_critical_check;

alter table public.odyssey_character_body_parts
  add constraint odyssey_character_body_parts_armor_critical_check
  check (armor_critical >= 0 and armor_max_critical >= 0);
...
create or replace function public.odyssey_refresh_character_combat_state(p_character_id uuid)
returns jsonb
language plpgsql
as $$
declare
...
begin
  ...
  select
    ...
    coalesce(bool_or(lower(coalesce(d.code, b.part_key)) in ('head', 'torso') and b.destroyed), false)
  into
    v_armor_summary,
    v_tracker_minor,
    v_tracker_serious,
  ...
  if not v_is_alive then
    v_is_conscious := false;
  else
    v_is_conscious := coalesce(v_existing_state.is_conscious, true);
  end if;
  ...
  values (
    ...
    v_body_summary,
    v_armor_summary,
    coalesce(v_existing_state.active_effects, '[]'::jsonb),
    coalesce(v_existing_state.active_penalties, '[]'::jsonb),
...
create or replace function public.odyssey_trim_combat_log(
  p_encounter_id uuid,
  p_room_id text
)
returns integer
...
begin
  if p_encounter_id is not null then
    with old_rows as (
      select id
      from public.odyssey_combat_log
      where encounter_id = p_encounter_id
      order by created_at desc, id desc
      offset 500
    ),
    deleted as (
      delete from public.odyssey_combat_log
      where id in (select id from old_rows)
      returning 1
    )
    select count(*) into v_deleted_count from deleted;
  elsif coalesce(nullif(p_room_id, ''), '') <> '' then
    with old_rows as (
      select id
      from public.odyssey_combat_log
...
grant execute on function public.odyssey_trim_combat_log(uuid, text) to anon, authenticated;
