...
// gm-extension/screens/creator/creatorScreen.js
...
async function mountCreatorScreen({ root: root2, runtime: runtime2 }) {
  if (!(root2 instanceof HTMLElement)) {
    throw new Error("Creator screen root is missing.");
  }
  await waitForObrReady();
  root2.innerHTML = createMarkup();
  const creatorHost = root2.querySelector("[data-creator-screen-host]");
  ...
  root2.addEventListener("odyssey:tabshow", () => {
    void loadRoomSupabaseSettings().then((settings) => {
